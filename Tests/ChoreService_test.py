...
from .Utils import skip_pytest_if_version_lower_than
...
def test_create_chore_with_dst_single_commit(tm1, constants, chore4):
    # create chores

    c4 = tm1.chores.get(constants["chore_name4"])

    assert c4.start_time.datetime.hour == constants["start_time"].hour
    assert c4._start_time._datetime.replace(hour=0) == constants["start_time"].replace(hour=0, microsecond=0)
    assert c4._name == constants["chore_name4"]
    assert c4.active is True
    assert c4._dst_sensitivity is True
    assert c4._execution_mode == Chore.SINGLE_COMMIT
    assert c4._frequency._days == f"{constants['frequency_days']:02}"
    assert c4._frequency._hours == f"{constants['frequency_hours']:02}"
    assert c4._frequency._minutes == f"{constants['frequency_minutes']:02}"
    assert c4._frequency._seconds == f"{constants['frequency_seconds']:02}"
    for task1, task2 in zip(constants["tasks"], c4._tasks):
        assert task1 == task2

@skip_pytest_if_version_lower_than("12.7.00002.1")
def test_create_chore_with_dst_multi_commit(tm1, constants, chore5):
    
    c5 = tm1.chores.get(constants["chore_name5"])
    print(tm1.version)

    assert c5.start_time.datetime.hour == constants["start_time"].hour
    assert c5._start_time._datetime.replace(hour=0) == constants["start_time"].replace(hour=0, microsecond=0)
    assert c5._name == constants["chore_name5"]
    assert c5.active is True
    assert c5._dst_sensitivity is True
    assert c5._execution_mode == Chore.MULTIPLE_COMMIT
    assert c5._frequency._days == f"{constants['frequency_days']:02}"
    assert c5._frequency._hours == f"{constants['frequency_hours']:02}"
    assert c5._frequency._minutes == f"{constants['frequency_minutes']:02}"
    assert c5._frequency._seconds == f"{constants['frequency_seconds']:02}"
    for task1, task2 in zip(constants["tasks"], c5._tasks):
        assert task1 == task2
...
def test_update_chore_without_tasks(tm1, constants, chore1):
        # get chore
        c = tm1.chores.get(constants["chore_name1"])
        # update all properties
        # update start time
...
def test_update_chore_add_tasks(tm1, constants, process2, chore1):
    # get chore
    c = tm1.chores.get(constants["chore_name1"])
    # update all properties
    # update start time
...
def test_update_chore_remove_tasks(tm1, constants, process2, chore1 ):
    # get chore
    c = tm1.chores.get(constants["chore_name1"])
    # update all properties
    # update start time
